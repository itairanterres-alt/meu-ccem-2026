...
function ProgramaScreen() {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      {/* Day tabs */}
      <div style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px 7px',borderBottom:`1px solid ${C.linha}`,background:'#fff',flexShrink:0}}>
        ...
        <div style={{textAlign:'right',flexShrink:0}}>
          ...
          {markedCount>0&&<div style={{fontFamily:'JetBrains Mono,monospace',fontSize:9,color:C.azulSoft,marginTop:1}}>{markedCount} na agenda</div>}
        </div>
      </div>
            ...
            {s.falas.map((f,i)=>(
              <div key={i} style={{display:'flex',gap:10,padding:'8px 0',borderBottom:i<s.falas.length-1?`1px solid ${C.linhaSoft}`:'none'}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:12,color:f.isMe?C.ouro:C.cinza,fontWeight:f.isMe?600:400}}>
                    {f.palestrante}{f.isMe&&' ★'}{f.aConfirmar&&<em style={{color:C.cinza}}> (a confirmar)</em>}
                  </div>
...
function LiveStrip(){
  const s=useMemo(()=>Object.values(SESSOES).find(s=>s.isNow),[]);
  if(!s) return null;
  return <div style={{display:'flex',alignItems:'center',gap:7,margin:'8px 0 0',padding:'5px 8px',background:'#eef6ff',borderRadius:7,border:'1px solid #c2daf8'}}>
    <span style={{width:6,height:6,borderRadius:'50%',background:'#22c55e',boxShadow:'0 0 0 3px rgba(34,197,94,.2)',flexShrink:0}}/>
    <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:8.5,color:C.azulSoft,textTransform:'uppercase',letterSpacing:'0.06em',fontWeight:600,flexShrink:0}}>Agora</span>
    <span style={{fontSize:11.5,color:C.tinta,fontWeight:500,flex:1,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{s.titulo}</span>
    <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:9,color:C.cinza,flexShrink:0}}>{s.inicio}</span>
  </div>;
}
function AppHeader(){
...
function TabBar({ aba }){
  const tabs=[{id:'programa',icon:<IcoCal size={21}/>,lbl:'Programa'},{id:'assistente',icon:<IcoChat size={21}/>,lbl:'Assistente'},{id:'caderno',icon:<IcoBook size={21}/>,lbl:'Caderno'},{id:'info',icon:<IcoInfo size={21}/>,lbl:'Info'}];
