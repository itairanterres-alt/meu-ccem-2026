  ...
  return (
    <div style={{height:'100%',overflowY:'auto',background:C.papel}}>
      ...
      <div style={{background:C.azul,color:'#fff',padding:'32px 22px 26px',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:8.5,textTransform:'uppercase',letterSpacing:'0.12em',opacity:.5,marginBottom:12}}>
        ...
        <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:10,opacity:.65,marginBottom:22,lineHeight:1.8}}>
        ...
        {isBefore && (
          <div style={{display:'inline-flex',alignItems:'center',gap:14,background:'rgba(255,255,255,.1)',borderRadius:12,padding:'12px 18px'}}>
            <div style={{textAlign:'center'}}>
              ...
              <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:7.5,textTransform:'uppercase',letterSpacing:'0.1em',opacity:.65,marginTop:3}}>dias</div>
            </div>
            <div style={{width:1,height:38,background:'rgba(255,255,255,.18)'}}/>
            <div style={{fontSize:13,lineHeight:1.5,opacity:.9}}>
              para o CCEM 2026<br/>
              <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:9.5,opacity:.65}}>sexta 23/out · 08h00</span>
            </div>
          </div>
        )}
        ...
        {isDuring && live && (
          <div style={{display:'inline-flex',alignItems:'center',gap:10,background:'rgba(34,197,94,.14)',border:'1px solid rgba(34,197,94,.28)',borderRadius:12,padding:'12px 16px'}}>
            ...
            <div>
              <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:8,textTransform:'uppercase',letterSpacing:'0.1em',color:'#86efac',marginBottom:3}}>Acontecendo agora</div>
        ...
        {!isBefore && !isDuring && (
          <div style={{display:'inline-flex',alignItems:'center',gap:8,background:'rgba(255,255,255,.08)',borderRadius:10,padding:'10px 14px'}}>
            <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:10,opacity:.7,lineHeight:1.5}}>CCEM 2026 encerrado<br/>acervo disponível no Caderno</span>
          </div>
        )}
          ...
          <div style={{display:'flex',gap:8,marginTop:16}}>
            {markCount > 0 && (
              <div style={{display:'flex',alignItems:'center',gap:5,background:'rgba(255,255,255,.08)',borderRadius:8,padding:'5px 10px'}}>
                ...
                <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:9,color:C.ouro}}>{markCount} marcada{markCount!==1?'s':''}</span>
              </div>
            )}
            {capCount > 0 && (
              <div style={{display:'flex',alignItems:'center',gap:5,background:'rgba(255,255,255,.08)',borderRadius:8,padding:'5px 10px'}}>
                <IcoCapture size={11} color="rgba(255,255,255,.7)"/>
                <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:9,color:'rgba(255,255,255,.7)'}}>{capCount} nota{capCount!==1?'s':''}</span>
              </div>
            )}
          </div>
        )}
      ...
      <div style={{padding:'20px 16px 10px'}}>
        <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:8.5,textTransform:'uppercase',letterSpacing:'0.1em',color:C.cinza,marginBottom:12,fontWeight:600}}>Acesso rápido</div>
          ...
          {shortcuts.map(s=>(
            <button key={s.id} onClick={()=>go('#/'+s.id)}
              ...
              <div>
                ...
                <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:8.5,color:C.cinza}}>{s.sub}</div>
              </div>
            </button>
          ))}
        </div>
      ...
      {highlights.length > 0 && (
        <div style={{padding:'10px 16px 28px'}}>
          <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:8.5,textTransform:'uppercase',letterSpacing:'0.1em',color:C.cinza,marginBottom:12,fontWeight:600}}>Destaques editoriais</div>
          {highlights.map((s,i)=>(
            <div key={s.id} onClick={()=>go('#/sessao/'+s.id)}
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontFamily:'JetBrains Mono,monospace',fontSize:8,color:C.ouro,textTransform:'uppercase',letterSpacing:'0.08em',marginBottom:4}}>★ Destaque</div>
                <div style={{fontSize:12.5,fontWeight:600,color:C.tinta,lineHeight:1.3,marginBottom:3}}>{s.titulo}</div>
                <div style={{display:'flex',gap:6,alignItems:'center'}}>
                  <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:8.5,color:C.azulSoft,fontWeight:600}}>{s.inicio}–{s.fim}</span>
                  <span style={{fontFamily:'JetBrains Mono,monospace',fontSize:8,color:C.cinza}}>{s.dia}</span>
                </div>
              </div>
