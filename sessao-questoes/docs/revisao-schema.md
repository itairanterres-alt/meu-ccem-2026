...
2. ~~Numeração de UC diverge entre a taxonomia da skill e o manual oficial.~~ **RESOLVIDO (24/07, com os Manuais Docentes 2026/2).** A taxonomia sempre esteve certa: 4ª fase, UC1 = Proliferação Celular, UC2 = Saúde da Mulher/Sexualidade/Planejamento Familiar, UC3 = Doenças Resultantes da Agressão ao Meio Ambiente — confirmado em 3 pontos do Manual Docente 4ª fase 2026/2. A dúvida anterior veio de tratar a numeração interna de `oas_med_unidavi_2026_1.json` (`f04.uc01`/`uc02`/`uc03`) como se fosse validada pelo manual, quando na verdade é o banco de OAs que está com a numeração rotacionada para a 4ª fase (uc01↔uc03↔uc02, ver `docs/anexos/README.md` §"Correção de dado"). `uc_slug` da amostra revertido para `..._f04_uc01_proliferacao_celular`. **Ação pendente para a coordenação:** corrigir a rotação de UC nos SPs da 4ª fase em `oas_med_unidavi_2026_1.json` na fonte da skill — e, já que a causa era estrutural (chave de UC, não texto), vale conferir se a mesma rotação aparece em outras fases do banco de OAs antes de usá-lo para validar `oa_slugs` na importação (Passo 3).
